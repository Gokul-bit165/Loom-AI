...
export function MachinePerformanceMatrix({
  ...
  return (
    <div className="panel-saas space-y-4 h-full flex flex-col justify-between">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 pb-2 border-b border-surface-100">
        <div>
          <h3 className="font-semibold text-sm xl:text-base text-surface-900 uppercase tracking-wide">
            Fleet Performance Matrix ({machines.length} Active Units)
          </h3>
          <p className="text-xs text-surface-500 font-normal">
            Click any unit cell to inspect its unified operational intelligence dossier
          </p>
        </div>
      ...
      <div className="relative flex-1 flex flex-col justify-center">
        <div className="grid grid-cols-6 sm:grid-cols-10 md:grid-cols-12 lg:grid-cols-15 xl:grid-cols-20 gap-1.5 p-3.5 bg-surface-50 rounded-xl border border-surface-200/80">
          {machines.map((m) => {
            ...
            return (
              <button
                ...
                className={`h-8 sm:h-9 rounded flex items-center justify-center font-mono text-[10px] sm:text-xs font-semibold transition-all cursor-pointer shadow-2xs ${
                  isCrit
        ...
        {hoveredMachine && (
          <div className="mt-2.5 p-3 rounded-xl bg-surface-900 text-white text-xs flex items-center justify-between animate-fadeIn shadow-lg">
            <div className="flex items-center space-x-2">
              <span className="font-mono font-bold text-amber-300 text-sm">{hoveredMachine.machine_id}</span>
            ...
            <div className="flex items-center space-x-4">
              ...
              <span className="text-[11px] text-brand-300 underline font-medium cursor-pointer">Click to inspect →</span>
            </div>
          </div>
        )}
      </div>
