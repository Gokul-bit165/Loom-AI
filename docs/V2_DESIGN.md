...
| 0.2 | New Alembic migration from the corrected DDL in §2 (no TimescaleDB; includes `production_target`, `fabric_roll`, `vendor_unit_monthly_summary`) | `alembic upgrade head` succeeds against a clean DB; `alembic downgrade base` succeeds and is symmetric. **DONE** — `v2/backend/alembic/versions/65aaf397533a_v2_initial_schema.py`, verified upgrade→downgrade→upgrade against a live `loom_ai_v2` Postgres DB. Autogenerate missed Postgres ENUM-type drops on downgrade (tables dropped, types didn't); fixed by hand with explicit `sa.Enum(name=...).drop(bind, checkfirst=True)` calls for all 19 enum types. |
| 0.3 | Master seed script — **ATM only** for `loom`; units, sheds, loom_type, style, style_unit_crimp_monthly, shift_master, employee, reason_code, cost_master, production_target basis; vendor units get `vendor_unit_monthly_summary` rows only (loaded from the real July-2026 table, `source='CSV_IMPORT'`) | Row counts match §3 exactly (168+24 ATM looms only, 10 styles, 6 units); `register_confirmed=false` on all seeded looms; `test_vendor_summary_rows_match_source_exactly` passes; idempotent. **DONE** — `v2/backend/app/seed/masters.py`. One correction to the brief's own source data, documented in the module docstring rather than silently resolved: the itemized role-count table sums to 123 employees, not the "137" stated alongside it; used 123 (the more specific, itemized figure) rather than padding with 14 invented rows. Similarly the grade-count table sums to 122, one short of 123 — the 123rd employee (a SWEEPER) is seeded with `grade=None` rather than forcing an ungrounded 122nd grade assignment. |
| 0.4 | `formulas.py` with full test coverage — **built before the demo generator**, since the generator depends on the corrected `loom_efficiency_pct` definition (correction #1); building the generator first would bake in the wrong one | Every function in §4.1 has ≥3 tests (normal/zero-denominator/None-input); `test_loom_efficiency_reconciles_with_performance_and_utilization` passes. **DONE** — `v2/backend/app/analytics/formulas.py`, 42/42 tests passing (`v2/backend/tests/test_formulas.py`). |
| 0.5 | Demo data generator (beam_run, assignment, production_log, stop_event, fabric_roll) for **ATM only** per §3 of this doc, using `formulas.py` from 0.4 | `test_demo_atm_matches_vendor_mrm`, `test_demo_picks_per_metre_ratio`, `test_demo_efficiency_has_real_spread` all pass; generation is deterministic (fixed `numpy.random.default_rng(DEMO_SEED)`). **DONE** — `v2/backend/app/seed/demo_generator.py`. Two bugs found and fixed while getting reconciliation to pass: (1) style pools were scoped to the exact `loom_type_code` (810 vs 910 separately) instead of the airjet family, so half of ATM's airjet looms never had the dominant plain style available, skewing the picks/metre ratio ~8% off target — fixed by pooling on make (airjet vs Sulzer); (2) the fabric-roll generator rounded each shift's own output (~76 m) against the whole-roll threshold (~551 m), so nearly every row rounded to zero rolls — fixed by accumulating metres per loom across shifts in chronological order and doffing a roll each time the accumulator crosses the threshold. |
| 0.6 | Reconciliation test suite wired into CI | `pytest tests/test_demo_reconciliation.py` is a required check; failing it fails the build. **DONE** — 7 tests passing (ATM-vs-MRM totals, vendor-summary-matches-source, vendor-units-have-no-looms structural guard, picks/metre ratio, efficiency-has-real-spread, the loom_efficiency identity on real generated rows, roll-count total) + 1 deliberately `@pytest.mark.skip` (`test_demo_loss_causes_sum_to_headline` — needs the not-yet-built Morning Brief module; kept as a placeholder marking the contract that module must satisfy, not silently dropped). Worst-20-ATM-looms-by-`loom_efficiency_pct` printed and reviewed by eye: real tail from 55.5% to 59.5%, all well below the ~89.6% unit mean, several distinct looms of both loom types — not a flat list. |
| 0.7 | `docs/V2_DDL.sql`, `docs/V2_DEMO_GENERATOR.md`, `docs/V2_RULES.md` extracted from this design doc into their own reference files once implementation starts | Not started — deferred; the design doc sections remain the source of truth for now. |

**0.2–0.6 are green.** P1 (ingestion templates matching the mill's real sheets, Operations table with cohort column, Loom Detail, daily PDF/WhatsApp export) can start.
